...
The client responds with opcode `0x21` containing the round index and hash data. The response payload is opaque hash data whose format varies by round. Round 2 responses are significantly larger than others due to the number of ship `.pyc` files.
...
2+      bytes   hash_data (variable length, opaque)
```
