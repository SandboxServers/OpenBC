...
2       u32le   ref_hash (reference hash -- StringHash of gamever "60")
6       u32le   dir_hash (hash of the directory name)
10+     var     file_tree (see Checksum Response Tree Format below)
```
...
**Non-recursive file tree** (rounds 0, 1, 3):
```
[file_count:u16le]
...
**Recursive file tree** (rounds 2, 0xFF):
```
[file_count:u16le]
  repeated file_count times:
    [name_hash:u32le]
    [content_hash:u32le]
[subdir_count:u16le]
  repeated subdir_count times:
    [subdir_name_hash:u32le]  -- hash of the subdirectory name
    [sub_file_count:u16le]    -- number of files in this subdirectory
      repeated sub_file_count times:
        [name_hash:u32le]
        [content_hash:u32le]
```
...
The `ref_hash` is always `StringHash("60")` = `0x7E0CE243`, corresponding to the game version string. This appears to be a protocol version marker.
