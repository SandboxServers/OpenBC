...
Tractor beam events do NOT use a dedicated network opcode. Instead, they travel inside
**PythonEvent (opcode 0x06)** payloads, generated by the host and sent to all clients.

In a 2-player stock dedicated server trace with active tractor beam usage, 55 tractor
events were observed:
- 53× TGObjPtrEvent (factory 0x010C): tractor target updates (which ship is being tractored)
- 2× TGSubsystemEvent (factory 0x0101): tractor lock/unlock toggles

These events are embedded inside the PythonEvent payload and visible only in the message
trace (event system layer), not as separate opcodes in the packet trace. The host generates
these events when tractor beam state changes occur.

The tractor beam's charge/recharge state and force output propagate via the normal
StateUpdate subsystem health round-robin (flag 0x20), not through dedicated messages.
