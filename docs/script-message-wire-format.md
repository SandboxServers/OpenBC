...
Both categories travel inside **the same type 0x32 transport envelope** (see [transport-layer.md](transport-layer.md)). There is no separate transport type for script messages.
...
- [transport-layer.md](transport-layer.md) — Transport framing (type 0x32 envelope, flags_len, fragmentation)
- [transport-cipher.md](transport-cipher.md) — AlbyRules stream cipher
- [phase1-verified-protocol.md](phase1-verified-protocol.md) — Complete opcode table and protocol flow
- [join-flow.md](join-flow.md) — When MISSION_INIT_MESSAGE is sent during join
- [disconnect-flow.md](disconnect-flow.md) — Cleanup on player leave
